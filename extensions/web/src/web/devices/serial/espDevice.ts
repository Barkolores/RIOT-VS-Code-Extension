...
export class EspDevice extends SerialDevice implements FlashInterface{
    ...
    async flash(binaries: {[offset:string]: Uint8Array}, args: string): Promise<void> {
        ...
        await espLoader.main().then(value => console.log(value)).catch(e => {
            console.error(e);
            throw e;
        });
        await espLoader.writeFlash(flashOptions).catch(async (e) => {
            await loaderOptions.transport.disconnect();
            throw e;
        });
